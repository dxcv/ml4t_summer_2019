...
class DTLearner:
    ...
    def build_tree(x, y, depth=0, leaf_size=1, verbose=False):
        ...
        split_value = np.median(x[:, feature])
        # TODO When all go to one tree, because median is not in the middle, infinite recursion happens
        #  Start here!!!
        left_tree = DTLearner.build_tree(x[x[:, feature] < split_value, :], y[x[:, feature] < split_value], depth + 1, leaf_size, verbose)
        right_tree = DTLearner.build_tree(x[x[:, feature] >= split_value, :], y[x[:, feature] >= split_value],
...
#
# predictions = model.query(train.iloc[:, :-1].values)
#
